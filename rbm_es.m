...
detail_string=sprintf('hls_%d_k_%d_es_%d_lr_%.3f_maxepoch_%d',hidden_layer_size,CD,early_stopper_limit,learning_rate,max_epochs);
for epoch=1:max_epochs
    ...
    for t=1:m
        x=X(t,:)';
        [j,h,x_bar]=sampler_unif(x,W,b,c);
        [j_not_req,h_bar,x_not_req]=sampler_unif(x_bar,W,b,c);
        W=W+learning_rate*(h*x'-h_bar*x_bar');
        b=b+learning_rate*(h-h_bar);
