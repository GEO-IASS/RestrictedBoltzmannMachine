...
for epoch=1:max_epochs
    ...
    for t=1:m
        x=X(t,:)';
        dropout_mask=binornd(1,0.5,size(x));
        x=x.*dropout_mask; 
        z2=b+W*x;
        h_x=sigmoid(z2);%a2
        z3=c+W'*h_x;
    ...
    if prev_J_val<J_val
        if early_stopper==0
            ...
            fname=strcat('dae_theta_',detail_string,'.mat');
            save(fname,'thetas');
        end
...
fname=strcat('dae_dev_',detail_string,'.csv');        
csvwrite(fname,output);
fname=strcat('dae_img_',detail_string,'.png');
